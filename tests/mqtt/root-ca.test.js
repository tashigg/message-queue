...
// Tests are mostly identical to `distribution.test.js` but using the ports of brokers that should only connect later.
describe("publish to node 1, receive from node2", () => {
    test("synchronously", async () => {
        ...
        const client2 = await mqtt.connectAsync("mqtt://localhost:1886", { protocolVersion: 5 });
